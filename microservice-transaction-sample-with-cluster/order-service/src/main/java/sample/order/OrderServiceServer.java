        ...
        .addShutdownHook(
            new Thread(
                () -> {
                  ...
                  shutdown();
                  blockUntilShutdown();
                  // `service` is null when startup failed before it was constructed, which happens
                  // on every run of the documented setup flow: the containers come up before the
                  // schema is loaded, so the first attempts fail and this hook runs.
                  if (service != null) {
                    service.close();
                  }
